...
export class GigService implements OnDestroy {
  ...
  getGigsForArtists(artists: string[]): GigResource[] {
    ...
    artists.forEach(artist => this.allSubscriptions.push(
      this.getGigs(artist)
        .subscribe(gigs => {
  ...
  searchEventsUrl = artist => this.baseUrl + '/events/search?app_key=' + this.appKey + `&keywords=${artist}&category=music&location=Germany&date=Future`;
  ...
  private searchTextFields(gig: GigResource, term: string): boolean {
    ...
    if (this.containsIgnoreCase(gig.comment, term)) {
      return true;
    }
